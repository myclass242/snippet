...
void TcpServer::removeConnection(std::shared_ptr<TcpConnection> tcpConn)
{
    loop_->assertInLoopThread();

    std::string connName = tcpConn->name();
    auto ite = connMap_.find(connName);
    assert(ite != connMap_.end());
    LOG_INFO << "TcpServer::removeConnection " << connName;
